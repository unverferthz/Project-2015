...
function changeFlagToLoadNewMarkers() {
  loadNewIncidents = true;
}
...
to the map*/
function filterLoadedIncidents(){
  clearMarkers();
...
function updateMapWithNewValues(){  
  ...
  var year1Selector = document.getElementById('year1Selector');
  var year2Selector = document.getElementById('year2Selector');
  ...
  var year1 = parseInt(year1Selector.value);
  var year2 = parseInt(year2Selector.value);
  ...
  if(firstDateFirst)
  {
    loadNewMarkers(month1, day1, month2, day2, year1, year2);
  }
  else
  {
    loadNewMarkers(month2, day2, month1, day1, year1, year2);
  }
}
...
to store in global array*/
 ajaxRequest.onload = function(){
  ...
  for(var i=0; i < incidents.length; i++)
  {
    ...
    //After incidents loaded, filter through them to display the correct ones for the user applied filters
    filterLoadedIncidents();
 }
...
/*Pre-condition: None */
/*Post-condition: Set month dropdown to current month and fill up year drop down and set current year */
function setupDropdowns(){
  ...
  var yearStarted = 2015;
  var yearsRunning = currentYear - 2015;

  for (var i=0; i <= yearsRunning; i++){
     var yearNumber = 2015 + i;

    var newOption1 = document.createElement("option");
    newOption1.value = yearNumber;
    var textNode1 = document.createTextNode(yearNumber);
    newOption1.appendChild(textNode1);
    
    var newOption2 = document.createElement("option");
    newOption2.value = yearNumber;
    var textNode2 = document.createTextNode(yearNumber);
    newOption2.appendChild(textNode2);


    year1Selector.appendChild(newOption1);
    year2Selector.appendChild(newOption2);
  }
  
  year1Selector.value = currentYear;
  year2Selector.value = currentYear;
}
